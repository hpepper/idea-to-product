...
sub GenerateContextModel {
    ...
    # The list is generated by  looking for ComponentRelations Where
    #  ComponentAId == $nComponentId and the Style == $f_szContextModel.
    my %hComponentHashList = GenerateComponentDiagramAndLegendRecursively(
                                     $nComponentId,
                                     $f_szSubTypeName,
                                     $f_szContextModel,
                                     1
...
sub GenerateComponentDiagramAndLegendRecursively {
  ...
  if ( $nComponentRecursingLevelLeft  >= 0 ) {
    ...
    # TODO C Turn this into get relations id.
    my %hComponentInformation = GetComponentInformation($nComponentId);
    push(@{$hReturnComplexHashesList{'arEntityList'}}, \%hComponentInformation);
    if ( $nComponentRecursingLevelLeft  > 0 ) {
 ...
 #TODO V Get the realtion information from the 'ComponentRelation' element.
      ...
      Log("DDD             GenerateComponentDiagramAndLegendRecursively() nComponentRecursingLevelLeft==$nComponentRecursingLevelLeft Not >0\n");
    } # endif >0
  } else {
    Log("DDD             GenerateComponentDiagramAndLegendRecursively() nComponentRecursingLevelLeft==$nComponentRecursingLevelLeft Not >=0\n");
  } # endif.
  Log("DDD done GenerateComponentDiagramAndLegendRecursively()\n");
  return(%hReturnComplexHashesList);
...
sub GenerateSoftwareArchitectureDocumentation {
  ...
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szModuleTypeName, $f_szLayeredStyleName);
  ...
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szClientServerStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szConcurrencyStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szMasterSlaveStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szPeerToPeerStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szPipeAndFilterStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szProduceConsumeStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szComponentAndConnectorTypeName, $f_szPublishSubscribeStyleName);

  # Allocation
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szAllocationTypeName, $f_szImplementationStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szAllocationTypeName, $f_szImplementationStyleName);
  GenerateViewPacketByTypeAndStyle($xmlNode, \%hSwArcHashList, $f_szAllocationTypeName, $f_szWorkAssignmentStyleName);
